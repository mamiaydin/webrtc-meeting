...
/**
 * Taiga Talk P2P - Client component
 *
...
let recStartTime;
let recElapsedTime;
let mirotalkTheme = 'taiga'; // dark - grey ...
...
let movingPeerDataChannels = {}; // keep track of our peer moving on room data channels
...
function getHtmlElementsById() {
    ...
    // my video
    myVideo = getId(myPeerId + '_video');
    myVideoWrap = getId(myPeerId + '_videoWrap');
    myVideoAvatarImage = getId(myPeerId + '_avatar');
    // buttons Bar
...
/**
 * Send async Moving positions data through RTC Data Channels
 * @param {object} config data
 */
 async function sendToMovingDataChannel(config) {
    if (thereIsPeerConnections() && typeof config === 'object' && config !== null) {
        for (let peer_id in movingPeerDataChannels) {
            if (movingPeerDataChannels[peer_id].readyState === 'open')
                await movingPeerDataChannels[peer_id].send(JSON.stringify(config));
        }
    }
}
...
function welcomeUser() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our Taiga Talk Video Chat Meeting',
...
async function handleRTCDataChannels(peer_id) {
                    ...
                    try {
                        ...
                        switch (dataMessage.type) {
                            case 'chat':
                                ...
                                break;
                            case 'avatar':
                                handleDataChannelAvatar(dataMessage);
                                break;
                            case 'speech':
    ...
    createMovingPeerDataChannel(peer_id);
}
...
function setTheme(theme) {
    ...
    switch (mirotalkTheme) {
        ...
        case 'taiga':
            ...
            console.log("==============================");
            swalBackground = 'radial-gradient(#393939, #000000)';
            document.documentElement.style.setProperty('--body-bg', '../images/taiga.png');
            document.documentElement.style.setProperty('--msger-bg', 'radial-gradient(#393939, #000000)');
            document.documentElement.style.setProperty('--msger-private-bg', 'radial-gradient(#393939, #000000)');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#393939, #000000)');
            document.documentElement.style.setProperty('--box-shadow', '0px 8px 16px 0px rgba(0, 0, 0, 0.2)');
            ...
            document.documentElement.style.setProperty('--right-msg-bg', '#056162');
            document.documentElement.style.setProperty('--private-msg-bg', '#6b1226');
            document.body.style.background = "../images/taiga.png";
            break;
        // ...
        default:
...
async function loadLocalMedia(stream) {
    ...
    myVideoAvatarImage.setAttribute('id', myPeerId + '_avatar');
    ...
    myVideoNavBar.setAttribute('id', 'localNavBar');
    ...
    var elem = document.getElementById("localNavBar"); 
...
function AddDraggableVideoAndAvatarElements(){
     //mamiiiii
     if(myPeerId != null){
        var cam = document.getElementById(myPeerId + "_videoWrap");
        var dragObj = document.getElementById(myPeerId + "_video");
        var dragObjAvatar = document.getElementById(myPeerId + "_avatar");
        if(cam != null && dragObj != null){
...
async function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    if (peer_video_status && peer_screen_status) {
        ...
        setVideoPrivacyStatus(peer_id + "_video",false);
    }
...
function handleVideoPrivacyBtn(videoId, privacyBtnId) {
    let video;
    if(videoId.endsWith("_video")){
        video = getId(videoId);
    }
    else{
        video = getId(videoId + "_video");
    }
    ...
    if (useVideo && video && privacyBtn) {
        privacyBtn.addEventListener('click', () => {
            ...
            setVideoPrivacyStatus(video.id, isVideoPrivacyActive);
...
function setVideoPrivacyStatus(peerVideoId, peerPrivacyActive) {
    let video;
    if(peerVideoId.endsWith("_video")){
        video = getId(peerVideoId);
    }
    else{
        video = getId(peerVideoId + "_video");
    }
        ...
        btnPn.addEventListener('click', () => {
            ...
            if (isVideoPinned) {
                ...
                if (!isScreenStreaming) {
                    ...
                    videoPlayer.removecl = 'var(--video-object-fit)';
                    videoPlayer.classList.remove('videoDefault');
                }
                if (isScreen || videoPlayer.style.name == peerId + '_typeScreen'){
                    videoPlayer.style.objectFit = 'contain';
                    //videoPlayer.className="videoDefault";
                }
                videoMediaContainer.removeChild(cam);
                cam.className = 'Camera';
                cam.style.width = '0';
                cam.style.height = '0';
                cam.style.top = '900px';
...
async function refreshMyLocalStream(stream, localAudioTrackChange = false) {
    ...
    if (isScreenStreaming) {
        isVideoPrivacyActive = false;
        setVideoPrivacyStatus(myPeerId + '_video', isVideoPrivacyActive);
    }

    // adapt video object fit on screen streaming
    getId(myPeerId + '_video').style.objectFit = isScreenStreaming ? 'contain' : 'var(--video-object-fit)';
...
/**
 * handle Incoming Data Channel Avatar
 * @param {object} dataMessage chat messages
 */
 function handleDataChannelAvatar(dataMessage) {
    if (!dataMessage) return;
    var avatarImgElement = document.getElementById(dataMessage.peer_id + "_avatar");
 ...
 * @param {object} config data
 */
 function handleDataChannelMovingPeer(data) {
    if (!data) return;
...
function leaveFeedback() {
    Swal.fire({
        ...
        text: 'Do you want to rate your Taiga Talk experience?',
        confirmButtonText: `Yes`,
        denyButtonText: `No`,
        showClass: {
            popup: '',
 ...
 */
 function dragVideoElement(elmnt, dragObj) {
    ...
    if (dragObj) {
        // if present, the header is where you move the DIV from:
        dragObj.onmousedown = dragMouseDown;
        console.log('Moving with dragObj!');
    } else {
        // otherwise, move the DIV from anywhere inside the DIV:
        elmnt.onmousedown = dragMouseDown;
        console.log('Moving with element!');
    }
 ...
 * @param {object} dragObj children element to make father draggable (click + mouse move)
 */
 function mobileDragVideoElement(elmnt, dragObj) {
    ...
    if (dragObj) {
        // if present, the header is where you move the DIV from:
        dragObj.ontouchmove = mobileDragMouseDown;
        console.log('Moving with dragObj!');
    } else {
        // otherwise, move the DIV from anywhere inside the DIV:
        elmnt.ontouchmove = mobileDragMouseDown;
        console.log('Moving with element!');
    }
 ...
 * @param {string} peer_id socket.id
 */ 
 function createMovingPeerDataChannel(peer_id) {
    movingPeerDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('moving_peer_channel');
    movingPeerDataChannels[peer_id].onopen = (event) => {
        console.log('Moving Peer Channel Created', event);
    };
}
