...
/**
 * Taiga Talk P2P - Server component
 *
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('Taiga Talk get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('Taiga Talk get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
io.sockets.on('connect', async (socket) => {
    ...
    socket.on('join', async (config) => {
        ...
        let peer_megaphone_status = config.peer_megaphone_status;
        let peer_rec_status = config.peer_rec_status;
        let peer_privacy_status = config.peer_privacy_status;
        let peer_location = config.peer_location;
        ...
        peers[channel][socket.id] = {
            ...
            peer_megaphone_status: peer_megaphone_status,
                ...
                if (peers[room_id][peer_id]['peer_name'] == peer_name) {
                    switch (element) {
                        ...
                        case 'screen':
                            ...
                            break;
                        case 'megaphone':
                            peers[room_id][peer_id]['peer_megaphone_status'] = status;
                            break;
                        case 'rec':
                        ...
                        case 'privacy':
                            ...
                            break;
                        case 'location':
                            peers[room_id][peer_id]['peer_location'] = status;
                            log.debug('LOCATION CHANGED', status);
                            break;
                    }
                }
            }
